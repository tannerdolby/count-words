...
var WordFrequencies = /** @class */ (function () {
    ...
    WordFrequencies.prototype.countWords = function (document) {
        ...
        var words = document.split(" ")
            ...
            .map(function (word) {
            var cleanWordObj = word.match(_this.wordRegex);
            var cleanWord = cleanWordObj[0] != undefined ? cleanWordObj[0] : "";
            _this.frequencies[cleanWord] = { "frequency": 0, "usage": 0 };
            _this.wordList.push(word);
            return cleanWord;
        });
        this.totalWords = words.length;
        words.forEach(function (word) {
            console.log(_this.frequencies[word]);
            var wordData = _this.frequencies[word];
            wordData.frequency += 1;
            wordData.usage = wordData.frequency / _this.totalWords;
        });
        return this.frequencies;
    };
    WordFrequencies.prototype.printFrequencies = function () {
        for (var key in this.frequencies) {
            console.log("".concat(key, ": ").concat(this.frequencies[key]));
        }
    };
...
var frequenciesOne = wf.countWordsInFile("./src/words.md");
